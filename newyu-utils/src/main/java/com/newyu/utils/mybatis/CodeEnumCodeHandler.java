 ...
 * @since JDK 1.7+
 */
public class CodeEnumCodeHandler extends BaseTypeHandler<CodeEnum> {
    private final CodeEnum[] enums;

    public CodeEnumCodeHandler(Class<? extends CodeEnum> type) {
        if (type == null) {
     ...
     * @return code对应的枚举类
     */
    private CodeEnum locateEnumStatus(int code) {
        for (CodeEnum status : enums) {
